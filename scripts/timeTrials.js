...
function init1() {
	...
	TT1.ticker.add(function(delta) {
		if (TT1.start) {
			...
			if (
				mod((TT1.time - delta) * dayTime, pi2) >
				mod(TT1.time * dayTime, pi2)
			) {
				TT1.day++
...
function init_2() {
	...
	const s_bacteria_2 = PIXI.Sprite.fromImage(
		'/assets/timeTrials/bacteria.png'
	)
	...
	TT_2.ticker.add(function(delta) {
		if (TT_2.start) {
			...
			if (
				mod((TT_2.time - delta) * dayTime, pi2) >
				mod(TT_2.time * dayTime, pi2)
			) {
				TT_2.day++
...
function init_3() {
	...
	const moveHandler = event => {
		if (
			...
			between(
				event.data.global.x,
				width / 2 - (width / 2 - 50) + 54,
				width / 2 + (width / 2 - 50) - 6
			) &&
			between(
				event.data.global.y,
				height / 2 - (height / 2 - 50) - 35 + 54,
				height / 2 + (height / 2 - 50) - 35 - 6
			)
		) {
			const vel = Math.sqrt(
				Math.abs(event.data.global.x - TT_3.oldX) +
					Math.abs(event.data.global.y - TT_3.oldY)
			)

			TT_3.oldX = event.data.global.x
			TT_3.oldY = event.data.global.y
...
function init_4() {
	...
	for (let i = 0; i < 120; i++) {
		...
		s_timeline_1_4.array[i].y = height / 2 + 100
		...
		s_timeline_2_4.array[i].y = height / 2 + 100
	...
	TT_4.ticker.add(function(delta) {
		if (TT_4.start) {
			...
			s_timeline_2_4.array[mod(TT_4.timer / 8, 120)].tint =
				dotColors[s_dot_4.kind]
			if (s_dot_4.kind == 'red') {
				s_timeline_2_4.array[mod(TT_4.timer / 8, 120)].y = height / 2 + 100 - 1
				s_timeline_2_4.array[mod(TT_4.timer / 8, 120)].scale.y = 6
			} else {
				s_timeline_2_4.array[mod(TT_4.timer / 8, 120)].y = height / 2 + 100
				s_timeline_2_4.array[mod(TT_4.timer / 8, 120)].scale.y = 4
			}
			if (TT_4.timer / 8 > 120) {
				TT_4.timer = 0
				for (let i = 0; i < 120; i++) {
		...
		if (TT_5.start) {
			if (s_drum_5.hit) {
				if (TT_5.phase === 1) {
					...
					s_sound_5.x =
						s_sound_5.x + Math.asin(0.3) / 1.57079633 * 2.1

					if (
						s_vision_5.y >= height * 2 / 3 &&
						s_sound_5.y >= height * 2 / 3
					) {
						TT_5.phase = 2
						...
						s_vision_5.target_d =
							...
							(s_sound_5.x - s_vision_5.x) /
								2 *
								Math.max(0, TT_5.habituation)
		...
		if (TT_6.start) {
			if (s_finger_6.hit) {
				if (TT_6.phase === 1) {
					...
					if (
						s_vision_6.y >= height * 2 / 3 &&
						s_sound_6.y >= height * 2 / 3
					) {
						TT_6.phase = 2
		...
		if (TT_7.start) {
			if (s_finger_7.hit) {
				if (TT_7.phase === 1) {
					...
					if (
						s_vision_7.y >= height * 2 / 3 &&
						s_sound_7.y >= height * 2 / 3
					) {
						TT_7.phase = 2
						...
						s_vision_7.target_d = Math.max(
							147,
							147 * 1.7 * TT_7.habituation
						)
